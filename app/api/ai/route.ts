...
export async function POST(req:Request){
 try{
  ...
  const safeConversation=Array.isArray(conversation)?conversation.slice(-20).map((m:any)=>({from:String(m.from||''),date:String(m.date||''),content:String(m.content||'').slice(0,800)})):[]
  ...
  const prompt=mode==='followup'?`Write a context-aware follow-up based primarily on the actual previous conversation below. Refer naturally to the last relevant message or unresolved point when supported. Do not restart the conversation, repeat the original message, or invent a reply or relationship. If the last message contains a question, request, or specific topic, acknowledge it naturally. If there is no usable history, say nothing that falsely implies a prior exchange. Context:\n${JSON.stringify(context)}`:`Write a personalized first LinkedIn outreach message. Mention one specific supported reason for contacting this person and make the ask lightweight (connect, learn, or brief chat). Do not make it sound like a job application unless supported. Context:\n${JSON.stringify(context)}`
  const response=await fetch('https://api.groq.com/openai/v1/chat/completions',{method:'POST',headers:{'Content-Type':'application/json',Authorization:`Bearer ${key}`},body:JSON.stringify({model:MODEL,temperature:.65,max_completion_tokens:512,reasoning_effort:'low',include_reasoning:false,messages:[{role:'system',content:system},{role:'user',content:prompt}]})})
  ...
  const text=typeof rawContent==='string' ? rawContent.trim() : Array.isArray(rawContent) ? rawContent.map((part:any)=>typeof part==='string'?part:String(part?.text||'')).join('').trim() : String(rawContent?.text||'').trim()
