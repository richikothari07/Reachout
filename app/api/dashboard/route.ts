...
export async function GET(req:Request){
 try{
  const u=new URL(req.url); const userId=u.searchParams.get('userId')||''; const target=u.searchParams.get('target')||'Product Manager'; const keywords=u.searchParams.get('keywords')||''; const scoringTarget=[target,keywords].filter(Boolean).join(' ')
  ...
  const people=connections.map((c:any)=>classify(c,scoringTarget,stats.get(key(c.linkedin_url))||stats.get(key(`${c.first_name} ${c.last_name}`)))).sort((a:any,b:any)=>b.score-a.score)
