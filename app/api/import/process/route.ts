...
export async function POST(req:Request){
  ...
  try{
    ...
    if(type==='connections'){
      ...
      const rows=parsed.data.map(x=>({user_id:userId,import_id:importId,linkedin_url:String(x.URL||'').trim(),email:String(x['Email Address']||'').trim(),dedupe_key:(String(x.URL||'').trim().toLowerCase()||`${String(x['First Name']||'').trim()}|${String(x['Last Name']||'').trim()}|${String(x.Company||'').trim()}`.toLowerCase()),first_name:String(x['First Name']||'').trim(),last_name:String(x['Last Name']||'').trim(),company:String(x.Company||'').trim(),position:String(x.Position||'').trim(),connected_on:String(x['Connected On']||'').trim()})).filter(x=>x.first_name||x.last_name)
    ...
    const parsed=Papa.parse<Record<string,string>>(body,{header:true,skipEmptyLines:true,transformHeader:h=>h.replace(/^\uFEFF/,'').trim()})
    const rows=parsed.data.map(x=>({user_id:userId,import_id:importId,conversation_id:String(x['CONVERSATION ID']||'').trim(),dedupe_key:`${String(x['CONVERSATION ID']||'').trim()}|${String(x.DATE||'').trim()}|${String(x.FROM||'').trim()}|${String(x.TO||'').trim()}|${String(x.CONTENT||'')}`.toLowerCase(),sender_name:String(x.FROM||'').trim(),sender_url:String(x['SENDER PROFILE URL']||'').trim(),recipient_name:String(x.TO||'').trim(),recipient_urls:String(x['RECIPIENT PROFILE URLS']||'').trim(),conversation_title:String(x['CONVERSATION TITLE']||'').trim(),message_date:String(x.DATE||'').trim(),subject:String(x.SUBJECT||'').trim(),content:String(x.CONTENT||''),folder:String(x.FOLDER||'').trim(),attachments:String(x.ATTACHMENTS||'').trim()})).filter(x=>x.sender_name||x.recipient_name||x.content)
