...
export default function Home(){
 ...
 const navigateTab=(next:typeof validTabs[number], replace=false)=>{
  ...
  if(typeof window!=='undefined'){
   ...
   if(current!==hash){
    ...
    window.history[method]({tab:next},'',`${window.location.pathname}${window.location.search}${hash}`)
   }
  }
 }
 useEffect(()=>{
  ...
  if((validTabs as readonly string[]).includes(fromHash))setTab(fromHash as typeof validTabs[number])
  else window.history.replaceState({tab:'Home'},'',`${window.location.pathname}${window.location.search}`)
  const onPopState=()=>{
   const next=decodeURIComponent(window.location.hash.replace(/^#/,''))
   setTab((validTabs as readonly string[]).includes(next)?next as typeof validTabs[number]:'Home')
  }
 ...
 const runVoiceCommand=async(command:string)=>{
  ...
  try{
   ...
   if(/^(go to|open|show) (home|dashboard)$/.test(lower)){navigateTab('Home');speak('Opening your home dashboard.');return}
   if(/who (should|do) i (follow.?up|follow up) with (next|now)?|who('s| is) next to follow up|what follow.?up should i do next|who should i contact next/.test(lower)){
    ...
    if(nextFollowup){navigateTab('Follow-ups');setSelected(nextFollowup);setVoiceGenerate(true);speak(`${nextFollowup.first_name} ${nextFollowup.last_name} is your next follow-up. I opened the conversation and I’m preparing a nudge based on your previous messages.`)}
      ...
      if(waiting){navigateTab('Follow-ups');setSelected(waiting);setVoiceGenerate(true);speak(`${waiting.first_name} ${waiting.last_name} is the closest nudge opportunity. I opened their conversation and I’m preparing a contextual follow-up.`)}
      else speak('There are no outstanding conversations to nudge right now.')
    } else speak('There are no outstanding conversations to nudge right now.')
    return
   }
   if(/(follow.?ups|follow ups)/.test(lower)&&/(show|open|go to|take me)/.test(lower)){navigateTab('Follow-ups');speak(`You have ${followups.length} nudges due.`);return}
   if(/(opportunit|people|contacts)/.test(lower)&&/(show|open|go to|take me)/.test(lower)){navigateTab('Opportunities');speak(`You have ${ranked.length} people in your network.`);return}
   if(/(outreach|messages)/.test(lower)&&/(show|open|go to|take me)/.test(lower)){navigateTab('Outreach');speak('Opening outreach.');return}
   ...
   const action=d.action; if(action==='navigate'){navigateTab(d.tab);speak(d.reply||'Done.');} else if(action==='select'){const person=findPerson(d.person||'');if(person){setSelected(person);speak(d.reply||`Opening ${person.first_name}.`)}else speak('I could not find that person.')} else if(action==='generate'){const person=findPerson(d.person||'');if(person){setSelected(person);setVoiceGenerate(true);speak(d.reply||`Preparing a personalized message for ${person.first_name}.`)}else speak('I could not find that person.')} else if(action==='contact'){const person=findPerson(d.person||'');if(person){markContacted(person);speak(d.reply||`Marked reach out complete for ${person.first_name}.`)}else speak('I could not find that person.')} else if(action==='followup'){const person=findPerson(d.person||'');if(person){toggleFollowedUp(person);speak(d.reply||`Marked follow-up complete for ${person.first_name}.`)}else speak('I could not find that person.')} else if(action==='search'){navigateTab('Opportunities');setQuery(d.query||'');speak(d.reply||`Searching for ${d.query||'that'}.`)} else if(action==='import'){setSetupOpen(true);speak(d.reply||'Opening import.')} else speak(d.reply||'I can help you navigate ReachOut, find people, mark outreach complete, and manage follow-ups.')
 ...
 return <main className="app" onDragOver={e=>{e.preventDefault();setDrag(true)}} onDragLeave={()=>setDrag(false)} onDrop={e=>{e.preventDefault();setDrag(false);importFiles(e.dataTransfer.files)}}>
  <aside className="sidebar">
   ...
   <nav>{([['Home',Sparkles],['Opportunities',Users],['Outreach',Zap],['Follow-ups',Clock3],['Live Intelligence',Globe2]] as const).map(([x,Icon])=><button key={x} className={`${tab===x?'nav active':'nav'} ${x==='Live Intelligence'?'navLive':'NavVisible'}`} onClick={()=>navigateTab(x as any)}><Icon size={17}/><span>{x==='Follow-ups'?'Follow ups':x==='Opportunities'?'Discover':x}</span>{x==='Follow-ups'&&followups.length>0&&<em>{followups.length}</em>}</button>)}</nav>
...
function HomeView({dashboardReady,hasData,target,connectionCount,messageCount,highPriority,hiringSignals,followups,recommended,goal,goalTarget,conversations,goalPct,setTab,navigateTab,setSelected,openImport,loading,markContacted,toggleFollowedUp,agentOpen,setAgentOpen,agentCommand,setAgentCommand,agentBusy,agentReply,agentActions,agentError,runAgent,findPerson,setVoiceGenerate}:{dashboardReady:boolean;hasData:boolean;target:string;connectionCount:number;messageCount:number;highPriority:number;hiringSignals:number;followups:Ranked[];recommended:Ranked[];goal:string;goalTarget:number;conversations:number;goalPct:number;setTab:any;navigateTab:(next:'Home'|'Opportunities'|'Outreach'|'Follow-ups'|'Live Intelligence')=>void;setSelected:any;openImport:()=>void;loading:boolean;markContacted:(p:Ranked)=>void;toggleFollowedUp:(p:Ranked)=>void;agentOpen:boolean;setAgentOpen:(v:boolean)=>void;agentCommand:string;setAgentCommand:(v:string)=>void;agentBusy:boolean;agentReply:string;agentActions:any[];agentError:string;runAgent:(c?:string)=>void;findPerson:(name:string)=>Ranked|undefined;setVoiceGenerate:(v:boolean)=>void}){
 const topPeople=recommended.slice(0,3)
 const topFollowups=followups.slice(0,3)
 return <>
  <section className="homeIntroCard">
   <div className="homeIntroCopy">
    <span className="sectionEyebrow">YOUR NETWORK, FOCUSED</span>
    <h2>{topPeople.length?<>I found <em>{topPeople.length} people</em> worth talking to.</>:<>Let’s find the people worth talking to.</>}</h2>
    <p>{topPeople.length?`Based on your ${target} goal, your network, and recent signals.`:`Import your LinkedIn data and I’ll turn it into a short, useful list.`}</p>
    <div className="homeQuickStats"><span><b>{connectionCount.toLocaleString()}</b> people mapped</span><span><b>{hiringSignals}</b> hiring signals</span><span><b>{conversations}</b> conversations started</span></div>
   </div>
   <div className="homeGoalMini">
    <span>YOUR CURRENT GOAL</span><b>{goal}</b><small>{target}</small>
    <button className="secondary" onClick={openImport}><Target size={14}/> Edit goal</button>
   </div>
  </section>

  <section className="askReachout">
   <div className="askTitle"><div><span className="sectionEyebrow">ASK REACHOUT</span><h2>What do you want to do next?</h2></div><span>Try asking naturally</span></div>
   <div className="askInput"><Sparkles size={17}/><input value={agentCommand} onChange={e=>setAgentCommand(e.target.value)} onKeyDown={e=>{if(e.key==='Enter'&&!agentBusy)runAgent()}} placeholder="e.g. Find 5 fintech PMs I should talk to"/><button className="primary" onClick={()=>runAgent()} disabled={agentBusy}>{agentBusy?'Working…':'Ask ReachOut'} <ArrowRight size={15}/></button></div>
   <div className="agentChips">{['Find hiring managers','Find alumni in product','Build my outreach plan','Who should I follow up with?'].map(x=><button key={x} onClick={()=>{setAgentCommand(x);runAgent(x)}} disabled={agentBusy}>{x}</button>)}</div>
   {agentError&&<div className="aiError">{agentError}</div>}
   {agentReply&&<div className="agentReply"><Sparkles size={14}/><span>{agentReply}</span></div>}
   {agentActions.length>0&&<div className="agentActions">{agentActions.slice(0,4).map((a:any,i)=><button className="agentAction" key={`${a.name}-${i}`} onClick={()=>{const person=findPerson(String(a.name||''));if(person){setSelected(person);setVoiceGenerate(a.action==='draft'||a.action==='followup')}}}><span className="agentPriority">{a.priority||i+1}</span><span className="agentActionMain"><b>{a.name}</b><small>{a.reason}</small></span><i>{a.action}</i><ChevronRight size={15}/></button>)}</div>}
   <small className="agentNote">Nothing is sent automatically. You stay in control.</small>
  </section>

  <section className="homePeopleSection">
   <div className="sectionHead"><div><span className="sectionEyebrow">PEOPLE I’D TALK TO</span><h2>Who should you reach out to?</h2><p>These are the people I’d put at the top of your list.</p></div><button className="textBtn" onClick={()=>navigateTab('Opportunities')}>See everyone <ChevronRight size={14}/></button></div>
   {topPeople.length?<div className="cards">{topPeople.map(p=><PersonCard key={`${p.linkedin_url}-${p.first_name}`} p={p} onClick={()=>setSelected(p)} onComplete={()=>p.action==='Follow up'?toggleFollowedUp(p):markContacted(p)}/>)}</div>:<Empty title="No people yet 👀" text="Import your LinkedIn data or adjust your goal and I’ll find some."/>}
  </section>

  <section className="homeToday">
   <div className="sectionHead"><div><span className="sectionEyebrow">YOUR NEXT MOVES</span><h2>Keep the momentum going.</h2><p>A tiny plan is better than another giant to-do list.</p></div></div>
   <div className="homeMoveGrid">
    <button className="homeMoveCard" onClick={()=>navigateTab('Outreach')}><div className="homeMoveIcon"><MessageSquare size={18}/></div><div><span>START A CONVERSATION</span><b>{topPeople.length||0} people ready to message</b><small>Pick one and start with context.</small></div><ChevronRight size={18}/></button>
    <button className="homeMoveCard" onClick={()=>navigateTab('Follow-ups')}><div className="homeMoveIcon mint"><Clock3 size={18}/></div><div><span>NUDGE A CONVERSATION</span><b>{topFollowups.length||0} follow-ups worth doing</b><small>{followups.length>3?`${followups.length-3} older conversations are tucked away.`:'You’re caught up for now.'}</small></div><ChevronRight size={18}/></button>
    <div className="homeProgressCard"><div><span>THIS GOAL</span><b>{conversations} / {goalTarget}</b></div><div className="progress light"><i style={{width:`${goalPct}%`}}/></div><small>{goalPct>=100?'Goal reached 🎉':`${Math.max(goalTarget-conversations,0)} more meaningful conversations to go.`}</small></div>
   </div>
  </section>
 </>
}
...
function QuickAction({p,onComplete}:{p:Ranked;onComplete:()=>void}){const label=p.action==='Follow up'?'Follow up':'Reach out';return <button className="quickAction" onClick={e=>{e.stopPropagation();onComplete()}} title={`Mark ${label.toLowerCase()} complete`} aria-label={`Mark ${label.toLowerCase()} complete`}><span className="quickCheck"><Check size={13}/></span></button>}
function PersonCard({p,onClick,onComplete}:{p:Ranked;onClick:()=>void;onComplete:()=>void}){const tags=[p.roleMatch?'Role fit':'',p.keywordMatches.length?'Keyword match':'',p.iitMatch?'Shared background':'',p.reputedStartupMatch?'Interesting company':'',p.lastOutgoing?'Already contacted':''].filter(Boolean).slice(0,3);return <div className="personCard" onClick={onClick} role="button" tabIndex={0} onKeyDown={e=>{if(e.key==='Enter'||e.key===' ')onClick()}}><div className="cardTop"><div className="avatar">{initials(p)}</div><div className="identity"><b>{p.first_name} {p.last_name}</b><span>{p.position||'Role not listed'}</span><small>{p.company||'Company not listed'}</small></div><span className="personArrow">↗</span></div><div className="whyTag"><Flame size={13}/><span><b>Why now:</b> {p.reasons[0]||'Strong match for your goal'}</span></div>{tags.length>0&&<div className="personTags">{tags.map((r,i)=><span key={`${r}-${i}`}>{r}</span>)}</div>}<div className="reasonList">{p.reasons.slice(1,3).map(r=><span key={r}>✓ {r}</span>)}</div><div className="cardAction"><span>{p.action==='Reach out'?'Start a conversation':p.action}</span><QuickAction p={p} onComplete={onComplete}/></div></div>}
...
function SetupModal({ownerName,setOwnerName,target,setTarget,keywords,setKeywords,goal,setGoal,goalTarget,setGoalTarget,files,importFiles,inputRef,removeFile,clearAll,close,save,profileFiles,importProfileFiles,clearProfile,onboarding=false}:{ownerName:string;setOwnerName:any;target:string;setTarget:any;keywords:string;setKeywords:any;goal:string;setGoal:any;goalTarget:number;setGoalTarget:any;files:FileItem[];importFiles:(f:FileList|File[])=>void;inputRef:any;removeFile:(id:string)=>void;clearAll:()=>void;close:()=>void;save:()=>void;profileFiles:{name:string;status:'ready'|'error';error?:string}[];importProfileFiles:(f:FileList|File[])=>void;clearProfile:()=>void;onboarding?:boolean}){
 ...
 const guide=[
  {title:'Open LinkedIn settings',text:'On desktop, go to LinkedIn → Me → Settings & Privacy → Data & Privacy.',ui:<div className="guideSimple"><div>LinkedIn</div><b>Settings & Privacy</b><span>Data & Privacy</span></div>},
  {title:'Download your data',text:'Choose Download your data and request your archive.',ui:<div className="guideSimple"><div>Data & Privacy</div><b>Download your data</b><span>Get a copy of your LinkedIn data</span></div>},
  {title:'Download the ZIP',text:'LinkedIn emails you when the archive is ready. Unzip it, then upload Connections.csv and messages.csv here.',ui:<div className="guideSimple"><Mail size={18}/><b>LinkedIn data.zip</b><span>Unzip before uploading</span></div>}
 ]
 const ready=files.some(f=>f.status==='ready')
 if(onboarding)return <div className="modalOverlay"><div className="setupModal onboardingSimple">
  <button className="modalClose" onClick={close} aria-label="Close"><X size={19}/></button>
  <span className="modalEyebrow">WELCOME TO REACHOUT</span><h2>Let’s find your people 👀</h2><p className="modalSub">Import your LinkedIn data, tell us what you want, and we’ll build a focused outreach plan.</p>
  <div className="onboardingMiniSteps"><span className={guideStep===0?'active':''}>1 Data</span><span className={guideStep===1?'active':''}>2 Upload</span><span className={guideStep===2?'active':''}>3 Goal</span></div>
  {guideStep<2?<div className="compactGuide"><div className="guideVisual">{guide[guideStep].ui}</div><div><span className="guideStepLabel">STEP {guideStep+1} OF 2</span><h3>{guide[guideStep].title}</h3><p>{guide[guideStep].text}</p></div></div>:<div className="simpleGoalForm"><label><span>Your goal</span><input value={goal} onChange={e=>setGoal(e.target.value)} placeholder="Land a Product role"/></label><div className="simpleGoalGrid"><label><span>Target role</span><input value={target} onChange={e=>setTarget(e.target.value)} placeholder="Product Manager"/></label><label><span>Keywords <em>optional</em></span><input value={keywords} onChange={e=>setKeywords(e.target.value)} placeholder="fintech, payments"/></label></div><label><span>Your name</span><input value={ownerName} onChange={e=>setOwnerName(e.target.value)} placeholder="Your name"/></label></div>}
  {guideStep===1&&<div className="simpleUpload"><UploadCloud size={20}/><div><b>Upload Connections.csv + messages.csv</b><span>Drop both files here or choose them from your device.</span></div><input ref={inputRef} type="file" accept=".csv,text/csv" multiple hidden onChange={e=>importFiles(e.target.files||[])}/><button className="primary" onClick={()=>inputRef.current?.click()}>Choose files</button></div>}
  {files.length>0&&<div className="modalFiles compactFiles">{files.map(f=><div className="modalFile" key={f.id}><div className={`fileStatus ${f.status}`}>{f.status==='processing'?<RefreshCw size={12} className="spin"/>:f.status==='ready'?<Check size={12}/>:<X size={12}/>}</div><div><b>{f.name}</b><span>{f.status==='ready'?`${f.rows.toLocaleString()} rows imported`:f.status==='processing'?'Processing…':f.error}</span></div>{f.status!=='processing'&&<button onClick={()=>removeFile(f.id)}><Trash2 size={14}/></button>}</div>)}</div>}
  <div className="cleanOnboardingActions"><button className="secondary" onClick={()=>{if(guideStep===0)close();else setGuideStep(guideStep-1)}}>{guideStep===0?'Close':'Back'}</button><button className="primary" disabled={guideStep===1&&!ready} onClick={()=>{if(guideStep<2)setGuideStep(guideStep+1);else save()}}>{guideStep===0?'Next':guideStep===1?(ready?'Next':'Upload files'):'Build my plan'} <ArrowRight size={14}/></button></div>
 </div></div>
 return <div className="modalOverlay"><div className="setupModal goalDataSimple">
  <button className="modalClose" onClick={close} aria-label="Close"><X size={19}/></button>
  <div className="modalEyebrow">GOAL & DATA</div><h2>Make ReachOut work for you.</h2><p className="modalSub">Set your goal, import your LinkedIn data, and you’re ready to go.</p>
  <div className="simpleGoalGrid"><label><span>What are you trying to do?</span><input value={goal} onChange={e=>setGoal(e.target.value)} placeholder="Land a Product role"/></label><label><span>Target role</span><input value={target} onChange={e=>setTarget(e.target.value)} placeholder="Product Manager"/></label></div>
  <div className="simpleGoalGrid"><label><span>Keywords <em>optional</em></span><input value={keywords} onChange={e=>setKeywords(e.target.value)} placeholder="fintech, payments, investing"/></label><label><span>Conversation target</span><input type="number" min="1" max="100" value={goalTarget} onChange={e=>setGoalTarget(Math.max(1,Number(e.target.value)||1))}/></label></div>
  <label className="simpleFullField"><span>Your name</span><input value={ownerName} onChange={e=>setOwnerName(e.target.value)} placeholder="Your name"/></label>
  <div className="simpleUpload"><UploadCloud size={20}/><div><b>Bring in your LinkedIn data</b><span>Connections.csv and messages.csv · you can upload both together.</span></div><input ref={inputRef} type="file" accept=".csv,text/csv" multiple hidden onChange={e=>importFiles(e.target.files||[])}/><button className="primary" onClick={()=>inputRef.current?.click()}>Choose files</button></div>
  {files.length>0&&<div className="modalFiles compactFiles">{files.map(f=><div className="modalFile" key={f.id}><div className={`fileStatus ${f.status}`}>{f.status==='processing'?<RefreshCw size={12} className="spin"/>:f.status==='ready'?<Check size={12}/>:<X size={12}/>}</div><div><b>{f.name}</b><span>{f.status==='ready'?`${f.rows.toLocaleString()} rows imported`:f.status==='processing'?'Processing…':f.error}</span></div>{f.status!=='processing'&&<button onClick={()=>removeFile(f.id)}><Trash2 size={14}/></button>}</div>)}</div>}
  {files.length>0&&<button className="dangerLink" onClick={clearAll}>Clear imported data</button>}
  <div className="modalFooter"><button className="secondary" onClick={close}>Cancel</button><button className="primary big" onClick={save}><Sparkles size={15}/> Save changes <ArrowRight size={15}/></button></div>
  <div className="privacyNote"><CheckCircle2 size={14}/> Your data stays in your ReachOut workspace and can be cleared anytime.</div>
 </div></div>
}
function Drawer({selected,ownerName,target,profile,messages,close,markContacted,contacted,followedUp,toggleFollowedUp,apiFetch,autoGenerate}:{selected:Ranked;ownerName:string;target:string;profile:ProfileContext|null;messages:Message[];close:()=>void;markContacted:(p:Ranked)=>void;contacted:string[];followedUp:string[];toggleFollowedUp:(p:Ranked)=>void;apiFetch:(input:string,init?:RequestInit)=>Promise<Response>;autoGenerate?:boolean}){
 ...
 return <div className="overlay" onClick={close}><div className="drawer" onClick={e=>e.stopPropagation()}><button className="close" onClick={close}><X/></button>
  ...
  {selected.linkedin_url&&<div className="profileActions"><a href={selected.linkedin_url} target="_blank" rel="noreferrer"><ArrowUpRight size={12}/> View LinkedIn</a></div>}
