...
export default function Home(){
 ...
 const [authReady,setAuthReady]=useState(false),[signedIn,setSignedIn]=useState(false),[accountEmail,setAccountEmail]=useState(''),[accountName,setAccountName]=useState('Richi'),[stateLoaded,setStateLoaded]=useState(false),[showCompletedOpportunities,setShowCompletedOpportunities]=useState(false),[accountMenuOpen,setAccountMenuOpen]=useState(false)
 ...
 useEffect(()=>{if(!userId||!signedIn)return;const t=window.setTimeout(()=>loadDashboard(),350);return()=>window.clearTimeout(t)},[userId,ownerName,target,keywords,days,profile,signedIn])
 ...
 const people=opportunityPeople.filter(c=>{const q=`${c.first_name} ${c.last_name} ${c.company} ${c.position}`.toLowerCase().includes(query.toLowerCase());const f=filter==='All'||(filter==='Not contacted'?(c.outgoingCount===0&&c.incomingCount===0):filter==='High signal');return q&&f})
 ...
 return <main className="app" onDragOver={e=>{e.preventDefault();setDrag(true)}} onDragLeave={()=>setDrag(false)} onDrop={e=>{e.preventDefault();setDrag(false);importFiles(e.dataTransfer.files)}}>
  ...
  <section className="content">
   <header className="topbar">
    <div className="siteHeaderRow">
     <div className="brand brandTop"><img className="brandLogo" src="/reachout-logo.png" alt="ReachOut"/><div><b>ReachOut</b><span>OUTREACH INTELLIGENCE</span></div></div>
     <nav className="mainTabs">{(['Home','Opportunities','Outreach','Follow-ups'] as const).map(x=><button key={x} className={tab===x?'mainTab active':'mainTab'} onClick={()=>setTab(x as any)}><span>{x==='Follow-ups'?'Follow ups':x}</span>{x==='Follow-ups'&&followups.length>0&&<em>{followups.length}</em>}</button>)}</nav>
     <div className="topbarRight">{tab==='Home'&&<div className="topActions"><button className="refresh" onClick={()=>loadDashboard(3)} disabled={loading}><RefreshCw size={15} className={loading?'spin':''}/>{loading?'Refreshing':'Refresh'}</button><button className="primary" onClick={()=>setSetupOpen(true)}><Target size={15}/> Edit goal</button></div>}<div className="accountMenuWrap"><button className="accountButton" onClick={()=>setAccountMenuOpen(v=>!v)} aria-expanded={accountMenuOpen} aria-label={`Account menu for ${accountName||'your account'}`} title={accountName||'Account'}><span className="accountAvatar"><UserRound size={16}/></span></button>{accountMenuOpen&&<div className="accountMenu"><div className="accountMenuLabel">ACCOUNT</div><div className="accountMenuName">{accountName||'Account'}</div><div className="accountMenuEmail">{accountEmail||'Your account'}</div><button onClick={()=>{setAccountMenuOpen(false);setSetupOpen(true)}}><Settings2 size={14}/> Goal & data</button><button className="accountLogout" onClick={async()=>{setAccountMenuOpen(false);await supabaseBrowser.auth.signOut()}}><X size={14}/> Log out</button></div>}</div></div>
    </div>
    <div className="pageIntro"><div><h1>{tab==='Home'?'Who is worth talking to today?':tab}</h1><p>{tab==='Home'?`I found a few people worth your attention. Here’s why they stand out and what you can do next.`:tab==='Opportunities'?'People who stand out because of your goals, your network, and what is happening around them.':tab==='Outreach'?'Here are the people I think you should say hi to first.':'A few conversations that could use a little nudge.'}</p></div></div>
   </header>
  ...
  <VoiceBot open={voiceOpen} listening={voiceListening} transcript={voiceTranscript} reply={voiceReply} onToggle={toggleVoice} onClose={()=>setVoiceOpen(false)}/>
...
function HomeView({hasData,target,connectionCount,messageCount,highPriority,hiringSignals,followups,recommended,goal,goalTarget,conversations,goalPct,setTab,setSelected,openImport,loading,markContacted,toggleFollowedUp,agentOpen,setAgentOpen,agentCommand,setAgentCommand,agentBusy,agentReply,agentActions,agentError,runAgent,findPerson,setVoiceGenerate}:{hasData:boolean;target:string;connectionCount:number;messageCount:number;highPriority:number;hiringSignals:number;followups:Ranked[];recommended:Ranked[];goal:string;goalTarget:number;conversations:number;goalPct:number;setTab:any;setSelected:any;openImport:()=>void;loading:boolean;markContacted:(p:Ranked)=>void;toggleFollowedUp:(p:Ranked)=>void;agentOpen:boolean;setAgentOpen:(v:boolean)=>void;agentCommand:string;setAgentCommand:(v:string)=>void;agentBusy:boolean;agentReply:string;agentActions:any[];agentError:string;runAgent:(c?:string)=>void;findPerson:(name:string)=>Ranked|undefined;setVoiceGenerate:(v:boolean)=>void}){
 if(!hasData)return <section className="emptyHome"><div className="heroCard"><div className="heroCopy"><span className="sectionEyebrow">START WITH YOUR GOAL</span><h2>You don’t need to know who to contact. <em>ReachOut helps you figure it out.</em></h2><p>Start with your goal, then explore the people in your network who are most worth a conversation. ReachOut gives you the context behind each suggestion and a natural next step.</p><div className="heroActions"><button className="primary big" onClick={openImport}>See how it works <ArrowRight size={16}/></button><span><Check size={13}/> Start without uploading</span></div></div><div className="heroVisual"><div className="visualTop"><span>A SIMPLE WAY TO THINK ABOUT IT</span><span>REACHOUT</span></div><div className="visualPerson"><div className="avatar large">AS</div><div><b>Ananya Shah</b><span>Product Lead · Fintech</span></div></div><div className="visualReason"><strong>WHY</strong><span>Your goals overlap, and there’s a natural reason to start a conversation.</span></div><div className="visualButton"><span>WHAT TO SAY</span><ArrowRight size={14}/></div></div></div><div className="howItWorks"><div><span>01</span><b>Import</b><small>Bring your LinkedIn export.</small></div><ArrowRight/><div><span>02</span><b>Prioritize</b><small>Find the people who matter.</small></div><ArrowRight/><div><span>03</span><b>Act</b><small>Say hi, follow up, keep going.</small></div></div></section>
 return <>
...
function QuickAction({p,onComplete}:{p:Ranked;onComplete:()=>void}){const label=p.action==='Follow up'?'Follow up':'Reach out';return <button className="quickAction" onClick={e=>{e.stopPropagation();onComplete()}} title={`Mark ${label.toLowerCase()} complete`} aria-label={`Mark ${label.toLowerCase()} complete`}><span>{label==='Follow up'?'Nudge them →':'Say hi →'}</span><span className="quickCheck"><Check size={13}/></span></button>}
function PersonCard({p,onClick,onComplete}:{p:Ranked;onClick:()=>void;onComplete:()=>void}){return <div className="personCard" onClick={onClick} role="button" tabIndex={0} onKeyDown={e=>{if(e.key==='Enter'||e.key===' ')onClick()}}><div className="cardTop"><div className="avatar">{initials(p)}</div><div className="identity"><b>{p.first_name} {p.last_name}</b><span>{p.position||'Role not listed'}</span><small>{p.company||'Company not listed'}</small></div></div><div className="whyTag"><Flame size={13}/><span>{p.reasons[0]||'Strong match for your goal'}</span></div><div className="reasonList">{p.reasons.slice(1,3).map(r=><span key={r}>✓ {r}</span>)}</div><div className="cardAction"><span>{p.action==='Reach out'?'Say hi':p.action}</span><QuickAction p={p} onComplete={onComplete}/></div></div>}

function OutreachView({queue,contacted,setSelected,markContacted,toggleFollowedUp,goal}:{queue:Ranked[];contacted:string[];setSelected:any;markContacted:(p:Ranked)=>void;toggleFollowedUp:(p:Ranked)=>void;goal:string}){const done=queue.filter(p=>contacted.includes(p.linkedin_url||`${p.first_name}-${p.last_name}`)).length;return <><div className="outreachHero"><div><span className="sectionEyebrow">FIRST OUTREACH</span><h2>Who is ready for a first message?</h2><p>These people fit your goal and look like a natural first conversation.</p></div><div className="queueProgress"><strong>{done}/{queue.length}</strong><span>queue actions completed</span><div className="progress"><i style={{width:`${queue.length?Math.round(done/queue.length*100):0}%`}}/></div></div></div><div className="outreachGrid"><section><div className="sectionHead compact"><div><span className="sectionEyebrow">TODAY</span><h2>Your next best actions</h2></div><span className="queueCount">{queue.length} actions</span></div>{queue.length?<div className="queueList">{queue.map((p,i)=>{const key=p.linkedin_url||`${p.first_name}-${p.last_name}`;const isDone=contacted.includes(key);return <div className={`queueItem ${isDone?'done':''}`} key={`${key}-${i}`}><div className="queueNumber">{isDone?<Check size={13}/>:i+1}</div><div className="avatar">{initials(p)}</div><div className="queueMain" onClick={()=>setSelected(p)}><div><b>{p.first_name} {p.last_name}</b><span>{p.position||'Role not listed'}{p.company?` · ${p.company}`:''}</span></div><p><strong>Say hi</strong> · {p.reasons[0]||'A good match for your goal'}</p></div><button className="queueOpen" onClick={()=>setSelected(p)}>Open <ChevronRight size={14}/></button><QuickAction p={p} onComplete={()=>markContacted(p)}/></div>})}</div>:<Empty title="You’re all caught up 🫡" text="Bring in your LinkedIn data and I’ll find your next conversation."/>}</section></div></>}

function VoiceBot({open,listening,transcript,reply,onToggle,onClose}:{open:boolean;listening:boolean;transcript:string;reply:string;onToggle:()=>void;onClose:()=>void}){return <>{open&&<div className="voicePanel"><div className={`voiceOrb ${listening?'active':''}`}><Mic size={22}/></div><div className="voiceCopy"><b>{listening?'Listening…':'ReachOut Voice'}</b><span className="voiceHelp">Try a short command:</span><div className="voiceExamples"><span>“Show my nudges”</span><span>“Who should I say hi to?”</span><span>“Draft a message for Aditi”</span></div>{transcript&&<div className="voiceTranscript"><small>You said</small><p>“{transcript}”</p></div>}{reply&&<small className="voiceReply"><Volume2 size={11}/> {reply}</small>}</div><button className="voiceClose" onClick={onClose}><X size={14}/></button></div>}<button className={`voiceFab ${listening?'listening':''}`} onClick={onToggle} aria-label={listening?'Stop listening':'Start voice input'}>{listening?<Mic size={19}/>:<MicOff size={19}/>}</button></>}
...
function SetupModal({ownerName,setOwnerName,target,setTarget,keywords,setKeywords,goal,setGoal,goalTarget,setGoalTarget,files,importFiles,inputRef,removeFile,clearAll,close,save,profileFiles,importProfileFiles,clearProfile,onboarding=false}:{ownerName:string;setOwnerName:any;target:string;setTarget:any;keywords:string;setKeywords:any;goal:string;setGoal:any;goalTarget:number;setGoalTarget:any;files:FileItem[];importFiles:(f:FileList|File[])=>void;inputRef:any;removeFile:(id:string)=>void;clearAll:()=>void;close:()=>void;save:()=>void;profileFiles:{name:string;status:'ready'|'error';error?:string}[];importProfileFiles:(f:FileList|File[])=>void;clearProfile:()=>void;onboarding?:boolean}){
 ...
 return <div className="modalOverlay"><div className={`setupModal ${onboarding?'onboardingModalClean':''}`}>
  ...
  {onboarding ? <>
   <div className="newOnboardingHeader">
    <div>
     <div className="onboardingKicker">WELCOME TO REACHOUT</div>
     <h2>Let’s make your network easier to act on.</h2>
     <p className="modalSub">Tell ReachOut what you’re working toward. Then we’ll show you who to talk to, why they matter, and what to say.</p>
    </div>
    <div className="cleanStepCount">{onboardingStep+1} / 2</div>
   </div>
   <div className="cleanStepBar">
    {['Your goal','Your network'].map((name,i)=><React.Fragment key={name}><div className={`cleanStep ${i===onboardingStep?'current':''} ${i<onboardingStep?'complete':''}`}><span>{i<onboardingStep?<Check size={11}/>:i+1}</span><b>{name}</b></div>{i<1&&<i className={i<onboardingStep?'done':''}/>}</React.Fragment>)}
   </div>
   {onboardingStep===0 && <div className="cleanOnboardingStep">
    <div className="onboardingPreview">
     <div className="previewLabel">WHAT REACHOUT WILL HELP YOU DO</div>
     <div className="previewSteps">
      <div><span>01</span><b>Find the right people</b><small>Prioritize your network around your goal.</small></div>
      <div><span>02</span><b>Understand the why</b><small>See the signal that makes each person worth contacting.</small></div>
      <div><span>03</span><b>Know what to say</b><small>Get a natural starting point for the conversation.</small></div>
     </div>
    </div>
    <div className="modalFields cleanFields onboardingGoalFields">
     <label><span>What are you working toward?</span><input value={goal} onChange={e=>setGoal(e.target.value)} placeholder="Land a Product role"/></label>
     <label><span>Target role</span><input value={target} onChange={e=>setTarget(e.target.value)} placeholder="Product Manager"/></label>
     <label><span>What should ReachOut look for? <em>optional</em></span><input value={keywords} onChange={e=>setKeywords(e.target.value)} placeholder="fintech, payments, investing"/></label>
     <label><span>Your name</span><input value={ownerName} onChange={e=>setOwnerName(e.target.value)} placeholder="Your name"/></label>
    </div>
    <div className="cleanInfo"><CheckCircle2 size={15}/><span>You can change these anytime. Your goal simply helps ReachOut decide what deserves your attention first.</span></div>
   </div>}
   {onboardingStep===1 && <div className="cleanOnboardingStep">
    <div className="dataChoice">
     <div className="dataChoiceCopy">
      <div className="onboardingSpark"><UploadCloud size={16}/></div>
      <div><h3>Want ReachOut to work from your LinkedIn network?</h3><p>Upload your export for personalized recommendations. You can also skip this and explore the product first.</p></div>
     </div>
     <div className="onboardingUpload cleanUpload">
      <div className="uploadReadyIcon"><UploadCloud size={16}/></div>
      <div><span className="sectionEyebrow">OPTIONAL</span><b>Import your LinkedIn CSVs</b><small>Connections.csv and messages.csv. We’ll detect and process them automatically.</small></div>
      <input ref={inputRef} type="file" accept=".csv,text/csv" multiple hidden onChange={e=>importFiles(e.target.files||[])}/>
      <button className="secondary" onClick={()=>inputRef.current?.click()}><UploadCloud size={15}/> Choose files</button>
     </div>
    </div>
    ...
    <div className="skipExplore"><div><b>Not ready to upload?</b><span>No problem. ReachOut can still show you how the experience works.</span></div><span className="skipBadge">You can add data later</span></div>
    <div className="cleanInfo"><CheckCircle2 size={15}/><span>Your imported data stays in your ReachOut workspace and can be cleared anytime.</span></div>
   </div>}
   <div className="cleanOnboardingActions">
    <button className="secondary" onClick={()=>{if(onboardingStep===0)close();else setOnboardingStep(0)}}>{onboardingStep===0?'Skip for now':'Back'}</button>
    <button className="primary" onClick={()=>{if(onboardingStep===0)setOnboardingStep(1);else save()}}>{onboardingStep===0?'Continue':'Start exploring'} <ArrowRight size={14}/></button>
   </div>
  </> : <>
...
function Drawer({selected,ownerName,target,profile,messages,close,markContacted,contacted,followedUp,toggleFollowedUp,apiFetch,autoGenerate}:{selected:Ranked;ownerName:string;target:string;profile:ProfileContext|null;messages:Message[];close:()=>void;markContacted:(p:Ranked)=>void;contacted:string[];followedUp:string[];toggleFollowedUp:(p:Ranked)=>void;apiFetch:(input:string,init?:RequestInit)=>Promise<Response>;autoGenerate?:boolean}){
 ...
 return <div className="overlay" onClick={close}><div className="drawer" onClick={e=>e.stopPropagation()}><button className="close" onClick={close}><X/></button>
  ...
  <div className="nextAction"><span className="sectionEyebrow">RECOMMENDED NEXT ACTION</span><b>{isFollowup?'Nudge them now':'Say hi'}</b><p>{isFollowup?'You have already opened the conversation. A quick follow up keeps things moving.':'This person fits your goal. A short, specific message is a good next step.'}</p></div>
