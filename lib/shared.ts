...
export function termsFor(target:string){ const t=norm(target); for(const k of Object.keys(ROLE_SYNONYMS)) if(t.includes(k)) return ROLE_SYNONYMS[k]; return t.split(' ').filter(x=>x.length>2).concat(['product','pm']) }
...
export function classify(c:Connection,target:string,stats?:MessageStats){
  const pos=norm(c.position), terms=termsFor(target), hits=terms.filter(t=>pos.includes(t)).length
